---
title: "NL Salmon resource predictions"
theme: sandstone
format:
  html:
    css: styles.css
...
watersheds <- vect("data/spatial data/NL_subsubbasins.shp") |>  
    project("EPSG:4326 - WGS 84") |> 
    tidyterra::mutate(html = paste0("map_", gsub(" ", "%20", watershed), ".html"))

watersheds$html <- replace(watersheds$html, watersheds$watershed == "Exploits", "Exploits_map.html")
watersheds$html <- replace(watersheds$html, watersheds$watershed == "Red Indian Lake", "Exploits_map.html")
...
pal <- colorFactor(palette = c("#32B1DB","#007BBD"), domain = watersheds$color_id)
...
# adds information to popups for each reach wehn it is selected
  ...
  label_text <- glue('<a href={watersheds$html}> <b>{watersheds$watershed}</b></a>')|> 
...
NL_map <- leaflet(height = "85vh", width = "54vw", options = leafletOptions(zoomSnap = 0.25)) |> 
      addProviderTiles('Esri.WorldImagery') |> # adds base map

      # open map to this screen
      setView(-56, 49.25, zoom = 6.75) |> 
  
      #can recenter to that location by clicking button
      addHomeButton(ext = c(-63, 46, -49, 52), position = "topleft", group = "Recenter") |> 
      # create layers to add vectors to
      addMapPane("drainage", zIndex = 410) |> 
      
      # add polygons and lines
      ## exploits drainage area
      addPolygons(data = watersheds, color = ~pal(color_id), weight = 1, fillColor = ~pal(color_id), opacity = 0.7, fillOpacity = 0.3, options = pathOptions(pane = "drainage"), popup = ~label_text, highlightOptions = highlightOptions(
