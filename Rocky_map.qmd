...
D50.col[rocky_river$D50 > 16 & rocky_river$D50 < 64] <- "#00D458"

D50.col.leg <- D50.pal(seq(0, 176, by = 16))
D50.col.leg[seq(0, 176, by = 16) > 16 & seq(0, 176, by = 16) < 64] <- "#00D458"
...
"<b>Predicted rgavel size (D50): </b> {rocky_river$D50}<br/>",
"<b>Discharge: </b> {rocky_river$DIS_AV_CMS}<br/>",
"<b>Width: </b> {rocky_river$width}<br/>",
"<b>Gradient: </b> {rocky_river$sgr_dk_rav}<br/>") |> 
...
rocky_map <- leaflet(width = "100vw", height = "75vh", options = leafletOptions(zoomSnap = 0.25)) |> 
      ...
      addPolygons(data = rocky_drainage, color = "#236D9E", weight = 1.25, fillColor = "#236D9E", opacity = 0.4, fillOpacity = 0.3, options = pathOptions(pane = "drainage")) |>
  
      ## rivers of the rocky
      addPolylines(data = smooth(rocky_river, method = "ksmooth", smoothness = 0.5), color = "#236D9E", weight = 3/rocky_river$ORD_CLAS, options = pathOptions(pane = "river"), group = "exploits", opacity = 0.8) |>

        ### D50
      addPolylines(data = smooth(rocky_river, method = "ksmooth", smoothness = 0.5), color = D50.col, weight = 3/rocky_river$ORD_CLAS, opacity = 1, options = pathOptions(pane = "data_D50"), group = "D50", popup = ~label_text) |>
      ...
      ### discharge
      addPolylines(data = smooth(rocky_river, method = "ksmooth", smoothness = 0.5), color = ~discharge.pal(rocky_river$DIS_AV_CMS), weight = 3/rocky_river$ORD_CLAS, opacity = 1, options = pathOptions(pane = "data_disch"), group = "Discharge", popup = ~label_text) |>
      ...
      ### width
      addPolylines(data = smooth(rocky_river, method = "ksmooth", smoothness = 0.5), color = ~width.pal(rocky_river$width), weight = 3/rocky_river$ORD_CLAS, opacity = 1, options = pathOptions(pane = "data_width"), group = "Width", popup = ~label_text) |>
      ...
      ### gradient
      addPolylines(data = smooth(rocky_river, method = "ksmooth", smoothness = 0.5), color = ~gradient.pal(rocky_river$sgr_dk_rav/10000), weight = 3/rocky_river$ORD_CLAS, opacity = 1, options = pathOptions(pane = "data_grad"), group = "Gradient", popup = ~label_text) |>
...
\
[Click here to download the Rocky River HydroATLAS data shapefile](https://github.com/emmwilson/NL_watersheds/blob/0d9a7a8e31f9d6fc5a61aac211dbf85ce949138c/data/spatial%20data/rocky_hydroatlas.shp?raw=true)
